...
const posts = [
  [
    ["neuron"],
    ["title", "Neuron"],
    ["paragraph", "Элементами полносвязного слоя являются искусственные нейроны"],
    ["image", "imgs/neuron.png", "alt"],
    ["paragraph", "Математически нейрон представляет собой взвешенный сумматор, единственный выход которого определяется через его входы и матрицу весов."]
  ],
  [
    ["artificial_neural_network"],
    ["title", "Artificial neural network"],
    ["paragraph", "После нескольких прохождений свёртки изображения и уплотнения с помощью пулинга система перестраивается от конкретной сетки пикселей с высоким разрешением к более абстрактным картам признаков, как правило, на каждом следующем слое увеличивается число каналов и уменьшается размерность изображения в каждом канале. В итоге остаётся большой набор каналов, хранящих небольшое число данных, которые интерпретируются как абстрактные понятия, выявленные из исходного изображения."],
    ["paragraph", "Эти данные объединяются и передаются на нейронную сеть, которая также может состоять из нескольких слоёв. При этом слои уже утрачивают пространственную структуру и обладают сравнительно небольшой размерностью. На рисунке 1.4 схематически изображён пример данного слоя."],
    ["image", "imgs/Perceptron.png", "perceptron"],
    ["paragraph", "На данном рисунке  – данные, переданные после операций свёртки и субдискретизации,  – коэффициент смещения функции активации,  – выходные значения. В свёрточной нейронной сети таких слоёв может быть несколько (рисунок 1.5), в таком случае выходные сигналы одного слоя передаются на вход другому, пока слои данного типа не заканчиваются, после чего результат подаётся на выход."],
